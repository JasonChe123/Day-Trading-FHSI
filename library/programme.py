...
def launch_tws(project_dir, user_name: str, password: str):
    ...
    while True:
        element = search_image_on_screen(img_path, confidence=0.5, grayscale=True)
        if element:
            break
...
def search_image_on_screen(img_path: os.path, confidence: float, grayscale: bool,
                           max_scale: float = 2.0, scale_factor: float = 0.01):
    ...
    while True:
        ...
        if vector == 1:
            searching_w *= 1 + scale_factor
            searching_h *= 1 + scale_factor
        else:
            searching_w *= 1 - scale_factor
            searching_h *= 1 - scale_factor
